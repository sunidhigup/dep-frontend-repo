...
const FlowBuilderTop = ({ resetElements, nodes, edges, entityName, userRole }) => {
  ...
  const handleRunEntity = async () => {
    const response = await runMdmEntity(entityName);
      ...
      <Box className={classes.BulderComponent}>
        ...
        <Box className={classes.BuilderRightComponent}>
          <Button
            variant="contained"
            size="small"
            onClick={handleRunEntity}
            className="button-color"
            style={{ marginRight: 10 }}
            disabled={userRole === 'ROLE_reader'}
          >
            <PlayArrowIcon /> Run Entity
          </Button>
