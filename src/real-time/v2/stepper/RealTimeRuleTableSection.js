...
import { getAllCustomRule, getCustomRuleApi } from "../../../api's/CustomRuleApi";
...
const RuleTableSectionNew = ({ componentType, NextData, setNextData, step_tablename, stream_name }) => {
  ...
  const getTableRules = async () => {
    ...
    try {
      const response = await fetchTableRule(client.client_id, stream_name, step_tablename);
  ...
  const fetchAllCustomRule = async () => {
    try {
      const response = await getAllCustomRule();
  ...
  const createRuleJson = async (TimeStampID) => {
    ...
    if (response.status === 200) {
      ...
      try {
        fetchCustomRule = await getAllCustomRule();
      } catch (error) {
        // console.log(error)
      }
  ...
  const saveTableRule = async (TimeStampID, idt) => {
    try {
      ...
      const metadata = {
        id: `${client.client_name}_${stream_name}_${tablename}`,
      ...
      await createMetadata(client.client_id, metadata);
      ...
      if (response.status === 200) {
        ...
        await storeTableRuleJson(client.client_id, stream_name, filename, idt, jsonData);

        await createTableRule({ ...InitialTableRule, fields: NewUpdatedFields }); // db store
        ...
        <Stack direction="row" spacing={2}>
          ...
          {(componentType === 'ruleEngine' || componentType === 'PreProcessor') && (
            <Tooltip title="Add new column">
              <Button variant="outlined" color="primary" onClick={AddModal}>
