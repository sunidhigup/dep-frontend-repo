...
import RealTimeStep1 from '../../stepper/real-time-stepper/RealTimeStep1';
...
import RealTimeJobOnBoardStep2 from '../../stepper/real-time-stepper/RealTimeJobOnBoardStep2';
...
const RealTimeStep = () => {
  ...
  const handlefetchRule = async () => {
    try {
      const tab = {
        path: `${streamData.client_name}/${streamData.stream_name}/${streamData.table_name}/Rule_engine/Table-Json/${jsonFile}`,
      ...
      if (resp.status === 200) {
        const data = {
          ...
          path: `s3://${BUCKET_NAME}/${tab.path}`,
          generated: false,
        };

        let fetchTableRuleResponse = await fetchTableRules(clientId, streamName);
        if (fetchTableRuleResponse === -1) {
          const persistResponse = await createTableRule(data);
          if (persistResponse.status === 201) {
            fetchTableRuleResponse = await fetchTableRules(clientId, streamName);
