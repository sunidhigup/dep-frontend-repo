...
export default function Customer_Requested_users() {
    ...
    React.useEffect(() => {
        const getRequestedUserList = async () => {
            let res = await axios.get(`${BASEURL}/customeradmin/getrequesteduserlist`, {
                auth: {
    ...
    const rejectUserBtn = async (event, idx) => {
        ...
        let res1 = await axios.delete(`${BASEURL}/customeradmin/rejectuser`, {
            data: {
                email: user["email"]
            },
            auth: {
        ...
        if (res1.status === 200) {
            ...
            let res2 = await axios.get(`${BASEURL}/customeradmin/getrequesteduserlist`, {
                auth: {
                    username: admin_username,
                    password: admin_password
                }
    ...
    const approveUserBtn = async (event, idx) => {
        let user = RequestedUserList[idx]
        let res = await axios.post(`${BASEURL}/customeradmin/adduser`, {
            name: user["name"],
        ...
        if (res.status === 200) {
            ...
            if (res1.status === 200) {
                await axios.get(`${BASEURL}/customeradmin/getrequesteduserlist`, {
                    auth: {
                        username: admin_username,
                        password: admin_password
                    }
