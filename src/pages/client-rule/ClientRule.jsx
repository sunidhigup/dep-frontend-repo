...
import { Box, IconButton, Stack, MenuItem, Modal, TableFooter, TablePagination, TableSortLabel } from '@mui/material';
...
import {
  ...
  deleteClientRuleApi,
  getClientRuleApi,
  getClientRuleByRulenameApi,
  updateClientRuleApi,
} from "../../api's/ClientRuleApi";
...
const ClientRule = () => {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - clientRules.length) : 0;
  ...
  const handleAddRule = async (e) => {
    ...
    try {
      ...
      if (editEnabled) {
        ...
        response = await updateClientRuleApi(data);
        ...
        response = await createClientRuleApi(data);
      }

      if (response.status === 201 || response.status === 200) {
        enqueueSnackbar('Client Rule Saved!', {
          variant: 'success',
  ...
  const fetchClientRule = async () => {
    try {
      const response = await getClientRuleApi(batch.client_id);

      if (response.status === 200) {
        setClientRules(response.data);
      }
    } catch (error) {
      if (error.response.status === 404) {
        enqueueSnackbar(`No Client rules found!`, {
          variant: 'warning',
          autoHideDuration: 3000,
          anchorOrigin: { vertical: 'top', horizontal: 'right' },
        });
  ...
  const handleDeleteRule = async (e, id) => {
    e.preventDefault();
    setLoadBtn(true);
    const response = await deleteClientRuleApi(id);
  ...
  const handleFetchRuleByName = async (ruleName) => {
    try {
      const response = await getClientRuleByRulenameApi(batch.client_id, ruleName);
                  ...
                  <Table sx={{ minWidth: 700 }} aria-label="customized table">
                    ...
                    <TableBody>
                      {(rowsPerPage > 0
                        ...
                        : clientRules.sort(getComparator(order, orderBy))
                      ).map((row, i) => (
                        <>
                          {' '}
